<script>
    ...
    const handleGobernorDistrics = async (event)=>{
        ...
        setTimeout(() => {
            ...
            candidatesOnDistrict= allcandidates.filter(candidate => candidate.distrito_nacional.slug === event.target.dataset.district)
        }, 300);
...
<div class="has-text-centered my-6 has-text-black">
    <h1 class="is-size-2 has-text-weight-medium has-text-black">¿Vivís en {districts.map((district) => ' '+district.name ).join(', ').replace(/, ([^,]*)$/, ' o $1')}?</h1>
    ...
    <div class="districts-buttons is-flex is-justify-content-center is-flex-wrap-wrap">
        {#each districts as district}
            <button class="button is-outlined is-black is-rounded is-uppercase" data-district="{district.slug}" on:click={handleGobernorDistrics}>
                {district.name}
            </button>
            {/each}
        </div>
        ...
        <div class="carousel-wrapper">
            ...
            >
                ...
                {#each candidatesOnDistrict as candidate}
                    <GobernorCandidateCard {candidate} showListButton proposalDisbled={candidate.partido.district.slug === 'buenos-aires'}/>
                {/each}
            </Carousel>
        </div>
	{/if}
